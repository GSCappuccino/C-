...
namespace MyApp
{
    public partial class _703_f : Form,FormFather
    {
        /*
                                       * 查询数据块-
                                       */
         ...
         * -查询数据块
         */
        private string DB_table_name = "g3cxfs"; 
        public static object text_a0;//传A0的值
        private string FormId = "703";
        //数据库链接
        ...
        public _703_f()
        {
            ...
            _703_f.MyConn = new OleDbConnection(_703_f.ConnString);
            _703_f.MyConn.Open();
            _703_f.MyAd = new OleDbDataAdapter("select *  from g3cxfs ", _703_f.MyConn);
            _703_f.MyAd.Fill(MyData);
            objCommandBuilder = new OleDbCommandBuilder(_703_f.MyAd);
            Directory.CreateDirectory(".\\" + main.companyName + "\\wjgl\\703");
            ...
            if (MyData.Tables[0].Rows.Count >= 1)
            {
                ...
                a6.Text = MyData.Tables[0].Rows[i][5].ToString();
                a7.Text = MyData.Tables[0].Rows[i][6].ToString();
                a5.Text = MyData.Tables[0].Rows[i][7].ToString();
        ...
        private void _703_f_Load(object sender, EventArgs e)
        {
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            if (MyData.Tables[0].Rows.Count == 0)
                a0.Text = (70300001).ToString();
            else
            {
                OleDbDataAdapter MyAd00 = new OleDbDataAdapter("select a0  from g3cxfs order by a0 desc ", MyConn);
        ...
        private void button8_Click(object sender, EventArgs e)
        {
            for (int v = 0; v < MyData.Tables[0].Rows.Count; v++)
            {
                ...
                if (this.a0.Text == MyData.Tables[0].Rows[v][0].ToString())
                {
                    ...
                    string SQL = "delete from g3cxfs where a0= '" + this.a0.Text + "'";
        ...
        {
            this.Enabled = true;
            Directory.CreateDirectory(".\\" + main.companyName + "\\wjgl\\703\\" + a0.Text + "");

            string temp = System.IO.Directory.GetCurrentDirectory();// d当前运行路径

            //    MessageBox.Show(temp);
            string OrignFile;//word模板路径
            OrignFile = "\\baseDB\\商标书式\\7 评审复审\\3撤销注册商标复审申请书\\撤销注册商标复审申请书(首页).dot";
            ...
            object aa = temp + "\\" + main.companyName + "\\wjgl\\703\\" + a0.Text + "\\" + a0.Text + ".docx";
        ...
        {
            Process.Start(".\\" + main.companyName + "\\wjgl\\703");
        }
        ...
        {
            Process.Start(".\\baseDB\\商标书式\\7 评审复审\\3撤销注册商标复审申请书\\撤销注册商标复审申请书（正文样式）.doc");
        }

       public  void CleanApp()
        {
            //内存数据删除
            ((DataSet)_703_f.MyData_delete).Tables[0].Rows.RemoveAt(_703_f.i);


            // int a =;// 数据库删除
            string SQL = "delete * from g3cxfs where a0='" + ((TextBox)_703_f.text_a0).Text + "'";
            OleDbCommand NewCom = new OleDbCommand(SQL, _703_f.MyConn);
            ...
            if (File.Exists(@".\\" + main.companyName + "\\wjgl\\703\\" + ((TextBox)_703_f.text_a0).Text + "\\" + ((TextBox)_703_f.text_a0).Text + ".docx"))
            {
                // MessageBox.Show("存在文件");
                File.Delete(@".\\" + main.companyName + "\\wjgl\\703\\" + ((TextBox)_703_f.text_a0).Text + "\\" + ((TextBox)_703_f.text_a0).Text + ".docx");

            }

            if (Directory.Exists(@".\\" + main.companyName + "\\wjgl\\703\\" + ((TextBox)_703_f.text_a0).Text + ""))
            {
                // MessageBox.Show("存在文件夹");
                Directory.Delete(@".\\" + main.companyName + "\\wjgl\\703\\" + ((TextBox)_703_f.text_a0).Text + "");
            }
