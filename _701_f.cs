...
namespace MyApp
{
    public partial class _701_f : Form,FormFather
    {
        /*
                                      * 查询数据块-
                                      */
         ...
         * -查询数据块
         */
        private string DB_table_name = "g1bhfs"; 
        public static object text_a0;//传A0的值
        private string FormId = "701";
        //数据库链接
        ...
        public _701_f()
        {
            ...
            _701_f.MyConn = new OleDbConnection(_701_f.ConnString);
            _701_f.MyConn.Open();
            _701_f.MyAd = new OleDbDataAdapter("select *  from g1bhfs ", _701_f.MyConn);
            _701_f.MyAd.Fill(MyData);
            objCommandBuilder = new OleDbCommandBuilder(_701_f.MyAd);
            Directory.CreateDirectory(".\\" + main.companyName + "\\wjgl\\701");
        ...
        private void _701_f_Load(object sender, EventArgs e)
        {
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            if (MyData.Tables[0].Rows.Count == 0)
                a0.Text = (70100001).ToString();
            else
            {
                OleDbDataAdapter MyAd00 = new OleDbDataAdapter("select a0  from g1bhfs order by a0 desc ", MyConn);
        ...
        private void button8_Click(object sender, EventArgs e)
        {
            for (int v = 0; v < MyData.Tables[0].Rows.Count; v++)
            {
                ...
                if (this.a0.Text == MyData.Tables[0].Rows[v][0].ToString())
                {
                    ...
                    string SQL = "delete from g1bhfs where a0= '" + this.a0.Text + "'";
        ...
        {
            this.Enabled = true;
            Directory.CreateDirectory(".\\" + main.companyName + "\\wjgl\\701\\" + a0.Text + "");

            string temp = System.IO.Directory.GetCurrentDirectory();// d当前运行路径

            //    MessageBox.Show(temp);
            string OrignFile;//word模板路径
            OrignFile = "\\baseDB\\商标书式\\7 评审复审\\1驳回商标注册申请复审申请书\\驳回商标注册申请复审申请书（首页）.dot";
            ...
            object aa = temp + "\\" + main.companyName + "\\wjgl\\701\\" + a0.Text + "\\" + a0.Text + ".docx";

            object[] MyBM = new object[14];//创建一个书签数组
        ...
        {
            Process.Start(".\\" + main.companyName + "\\wjgl\\701");
        }
        ...
        {
            Process.Start(".\\baseDB\\商标书式\\7 评审复审\\1驳回商标注册申请复审申请书\\驳回商标注册申请复审申请书（正文样式）.doc");
        }

       public void CleanApp()
        {
            //内存数据删除
            ((DataSet)_701_f.MyData_delete).Tables[0].Rows.RemoveAt(_701_f.i);


            // int a =;// 数据库删除
            string SQL = "delete * from g1bhfs where a0='" + ((TextBox)_701_f.text_a0).Text + "'";
            OleDbCommand NewCom = new OleDbCommand(SQL, _701_f.MyConn);
            ...
            if (File.Exists(@".\\" + main.companyName + "\\wjgl\\701\\" + ((TextBox)_701_f.text_a0).Text + "\\" + ((TextBox)_701_f.text_a0).Text + ".docx"))
            {
                // MessageBox.Show("存在文件");
                File.Delete(@".\\" + main.companyName + "\\wjgl\\701\\" + ((TextBox)_701_f.text_a0).Text + "\\" + ((TextBox)_701_f.text_a0).Text + ".docx");

            }

            if (Directory.Exists(@".\\" + main.companyName + "\\wjgl\\701\\" + ((TextBox)_701_f.text_a0).Text + ""))
            {
                // MessageBox.Show("存在文件夹");
                Directory.Delete(@".\\" + main.companyName + "\\wjgl\\701\\" + ((TextBox)_701_f.text_a0).Text + "");
            }
