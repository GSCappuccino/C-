...
namespace MyApp
{
    public partial class _572_f : Form,FormFather
    {
        /*
                            * 查询数据块-
                            */
         ...
         * -查询数据块
         */
        private string DB_table_name = "e72cxzx"; 
        public static object text_a0;//传A0的值
        private string FormId = "572";
        //数据库链接
        ...
        public _572_f()
        {
            ...
            _572_f.MyConn = new OleDbConnection(_572_f.ConnString);
            _572_f.MyConn.Open();
            _572_f.MyAd = new OleDbDataAdapter("select *  from e72cxzx ", _572_f.MyConn);
            _572_f.MyAd.Fill(MyData);
            objCommandBuilder = new OleDbCommandBuilder(_572_f.MyAd);
            Directory.CreateDirectory(".\\" + main.companyName + "\\wjgl\\572");
        ...
        private void _572_f_Load(object sender, EventArgs e)
        {
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            if (MyData.Tables[0].Rows.Count == 0)
                a0.Text = (57200001).ToString();
            else
            {
                OleDbDataAdapter MyAd00 = new OleDbDataAdapter("select a0  from e72cxzx order by a0 desc ", MyConn);
        ...
        private void button8_Click(object sender, EventArgs e)
        {
            for (int v = 0; v < MyData.Tables[0].Rows.Count; v++)
            {
                ...
                if (this.a0.Text == MyData.Tables[0].Rows[v][0].ToString())
                {
                    ...
                    string SQL = "delete from e72cxzx where a0= '" + this.a0.Text + "'";
        ...
        private void save()
        {
            ...
            this.Enabled = true;
            Directory.CreateDirectory(".\\" + main.companyName + "\\wjgl\\572\\" + a0.Text + "");

            string temp = System.IO.Directory.GetCurrentDirectory();// d当前运行路径

            //    MessageBox.Show(temp);
            string OrignFile;//word模板路径
            OrignFile = "\\baseDB\\商标书式\\3 续展变更转让许可质押注销\\7注销\\2撤回商标注销申请书.dot";
            ...
            object aa = temp + "\\" + main.companyName + "\\wjgl\\572\\" + a0.Text + "\\" + a0.Text + ".docx";

            object[] MyBM = new object[22];//创建一个书签数组

            for (int i = 0; i < 10; i++)//给书签数组赋值
                MyBM[i] = "a" + (i + 1).ToString();
            MyBM[10] = "a11_1";
            MyBM[11] = "a11_2";
            for (int i = 12; i < 21; i++)//给书签数组赋值
                MyBM[i] = "a" + i.ToString();
            //给对应的书签位置写入数据
        ...
        private void button10_Click(object sender, EventArgs e)
        {
            ...
            {
                panel2.Enabled = false;
                OleDbDataAdapter MyAd0 = new OleDbDataAdapter("select top 1 * from e72cxzx order by a0 desc", MyConn);
        ...
        {
             Process.Start(".\\" + main.companyName + "\\wjgl\\572");
        }
        ...
        public void CleanApp()
        {
            ...
            //内存数据删除
            ((DataSet)_572_f.MyData_delete).Tables[0].Rows.RemoveAt(_572_f.i);


            // int a =;// 数据库删除
            string SQL = "delete * from e72cxzx where a0='" + ((TextBox)_572_f.text_a0).Text + "'";
            OleDbCommand NewCom = new OleDbCommand(SQL, _572_f.MyConn);
            ...
            if (File.Exists(@".\\" + main.companyName + "\\wjgl\\572\\" + ((TextBox)_572_f.text_a0).Text + "\\" + ((TextBox)_572_f.text_a0).Text + ".docx"))
            {
                // MessageBox.Show("存在文件");
                File.Delete(@".\\" + main.companyName + "\\wjgl\\572\\" + ((TextBox)_572_f.text_a0).Text + "\\" + ((TextBox)_572_f.text_a0).Text + ".docx");

            }

            if (Directory.Exists(@".\\" + main.companyName + "\\wjgl\\572\\" + ((TextBox)_572_f.text_a0).Text + ""))
            {
                // MessageBox.Show("存在文件夹");
                Directory.Delete(@".\\" + main.companyName + "\\wjgl\\572\\" + ((TextBox)_572_f.text_a0).Text + "");
            }
