...
namespace MyApp
{
    public partial class _571_f : Form,FormFather
    {
        /*
                            * 查询数据块-
                            */
         ...
         * -查询数据块
         */
        private string DB_table_name = "e71sbzx"; 
        public static object text_a0;//传A0的值
        private string FormId = "571";
        //数据库链接
        ...
        public _571_f()
        {
            ...
            _571_f.MyConn = new OleDbConnection(_571_f.ConnString);
            _571_f.MyConn.Open();
            _571_f.MyAd = new OleDbDataAdapter("select *  from e71sbzx ", _571_f.MyConn);
            _571_f.MyAd.Fill(MyData);
            objCommandBuilder = new OleDbCommandBuilder(_571_f.MyAd);
            Directory.CreateDirectory(".\\" + main.companyName + "\\wjgl\\571");
        ...
        private void _571_f_Load(object sender, EventArgs e)
        {
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            if (MyData.Tables[0].Rows.Count == 0)
                a0.Text = (57100001).ToString();
            else
            {
                OleDbDataAdapter MyAd00 = new OleDbDataAdapter("select a0  from e71sbzx order by a0 desc ", MyConn);
        ...
        private void button8_Click(object sender, EventArgs e)
        {
            for (int v = 0; v < MyData.Tables[0].Rows.Count; v++)
            {
                ...
                if (this.a0.Text == MyData.Tables[0].Rows[v][0].ToString())
                {
                    ...
                    string SQL = "delete from e71sbzx where a0= '" + this.a0.Text + "'";
        ...
        {
            this.Enabled = true;
            Directory.CreateDirectory(".\\" + main.companyName + "\\wjgl\\571\\" + a0.Text + "");

            string temp = System.IO.Directory.GetCurrentDirectory();// d当前运行路径

            //    MessageBox.Show(temp);
            string OrignFile;//word模板路径
            OrignFile = "\\baseDB\\商标书式\\3 续展变更转让许可质押注销\\7注销\\1商标注销申请书.dot";
            ...
            object aa = temp + "\\" + main.companyName + "\\wjgl\\571\\" + a0.Text + "\\" + a0.Text + ".docx";
        ...
        private void button10_Click(object sender, EventArgs e)
        {
            ...
            {
                panel2.Enabled = false;
                OleDbDataAdapter MyAd0 = new OleDbDataAdapter("select top 1 * from e71sbzx order by a0 desc", MyConn);
        ...
        {
            Process.Start(".\\" + main.companyName + "\\wjgl\\571");
        }
        ...
        {
            //内存数据删除
            ((DataSet)_571_f.MyData_delete).Tables[0].Rows.RemoveAt(_571_f.i);


            // int a =;// 数据库删除
            string SQL = "delete * from e71sbzx where a0='" + ((TextBox)_571_f.text_a0).Text + "'";
            OleDbCommand NewCom = new OleDbCommand(SQL, _571_f.MyConn);
            ...
            if (File.Exists(@".\\" + main.companyName + "\\wjgl\\571\\" + ((TextBox)_571_f.text_a0).Text + "\\" + ((TextBox)_571_f.text_a0).Text + ".docx"))
            {
                // MessageBox.Show("存在文件");
                File.Delete(@".\\" + main.companyName + "\\wjgl\\571\\" + ((TextBox)_571_f.text_a0).Text + "\\" + ((TextBox)_571_f.text_a0).Text + ".docx");

            }

            if (Directory.Exists(@".\\" + main.companyName + "\\wjgl\\571\\" + ((TextBox)_571_f.text_a0).Text + ""))
            {
                // MessageBox.Show("存在文件夹");
                Directory.Delete(@".\\" + main.companyName + "\\wjgl\\571\\" + ((TextBox)_571_f.text_a0).Text + "");
            }
