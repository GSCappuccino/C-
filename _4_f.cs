...
namespace MyApp
{
    public partial class _4_f : Form,FormFather
    {
        /*
* 查询数据块-
*/
         ...
         * * -查询数据块
        */
        private string DB_table_name = "dtsbz"; 
        public object text_a0;//传A0的值
        private string FormId="4";
        //数据库链接
        ...
        public _4_f()
        {
            ...
            main.text_19 = this.a12;
            main.text_20 = this.a13;
            main.text_21 = this.a14;
            main.text_22 = this.a15;
            text_a0 = this.a0;
            MyData_delete = MyData;
            Control.CheckForIllegalCrossThreadCalls = false;
            MyConn = new OleDbConnection(ConnString);
            MyConn.Open();
            MyAd = new OleDbDataAdapter("select *  from dtsbz ",MyConn);
            MyAd.Fill(MyData);
            objCommandBuilder = new OleDbCommandBuilder(MyAd);
            Directory.CreateDirectory(".\\" + main.companyName + "\\wjgl\\4");
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            if (MyData.Tables[0].Rows.Count == 0)
                a0.Text = (400001).ToString();
            else
            {

                OleDbDataAdapter MyAd00 = new OleDbDataAdapter("select a0  from dtsbz order by a0 desc ", MyConn);
        ...
        private void save()
        {
            ...
            Directory.CreateDirectory(".\\" + main.companyName + "\\wjgl\\4\\" + a0.Text + "");

            string temp = System.IO.Directory.GetCurrentDirectory();// d当前运行路径

            //    MessageBox.Show(temp);
            string OrignFile;//word模板路径
            OrignFile = "\\baseDB\\商标书式\\4 特殊标志申请\\特殊标志登记申请书.dot";
            ...
            object aa = temp + "\\" + main.companyName + "\\wjgl\\4\\" + a0.Text + "\\" + a0.Text + ".docx";
        ...
        private void button10_Click(object sender, EventArgs e)
        {
            ...
            {
                panel2.Enabled = false;
                OleDbDataAdapter MyAd0 = new OleDbDataAdapter("select top 1 * from dtsbz order by a0 desc", MyConn);
                DataSet MyData0 = new DataSet();
                MyAd0.Fill(MyData0);
        ...
        {
            OleDbDataAdapter MyAd0 = new OleDbDataAdapter("select *  from dtsbz ", MyConn);
            DataSet MyData0 = new DataSet();
        ...
        {
            Process.Start(".\\" + main.companyName + "\\wjgl\\4");
        }

        private void _4_f_Load(object sender, EventArgs e)
        {
      ...
      public void CleanApp()
        {
            ...
            // int a =;// 数据库删除
            string SQL = "delete * from dtsbz where a0='" + ((TextBox)text_a0).Text + "'";
            ...
            if (File.Exists(@".\\" + main.companyName + "\\wjgl\\4\\" + ((TextBox)text_a0).Text + "\\" + ((TextBox)text_a0).Text + ".docx"))
            {
                // MessageBox.Show("存在文件");
                File.Delete(@".\\" + main.companyName + "\\wjgl\\4\\" + ((TextBox)text_a0).Text + "\\" + ((TextBox)text_a0).Text + ".docx");

            }

            if (Directory.Exists(@".\\" + main.companyName + "\\wjgl\\4\\" + ((TextBox)text_a0).Text + ""))
            {
                // MessageBox.Show("存在文件夹");
                Directory.Delete(@".\\" + main.companyName + "\\wjgl\\4\\" + ((TextBox)text_a0).Text + "");
            }
