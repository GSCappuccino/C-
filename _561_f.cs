...
namespace MyApp
{
    public partial class _561_f : Form,FormFather
    {
        /*
               * 查询数据块-
               */
         ...
         * -查询数据块
         */
        private string DB_table_name = "e61zrzy"; 
        public static object text_a0;//传A0的值
        private string FormId="561";
        //数据库链接
        ...
        public _561_f()
        {
            ...
            _561_f.MyConn = new OleDbConnection(_561_f.ConnString);
            _561_f.MyConn.Open();
            _561_f.MyAd = new OleDbDataAdapter("select *  from e61zrzy ", _561_f.MyConn);
            _561_f.MyAd.Fill(MyData);
            objCommandBuilder = new OleDbCommandBuilder(_561_f.MyAd);
            Directory.CreateDirectory(".\\" + main.companyName + "\\wjgl\\561");
        ...
        private void _561_f_Load(object sender, EventArgs e)
        {
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            if (MyData.Tables[0].Rows.Count == 0)
                a0.Text = (56100001).ToString();
            else
            {
                OleDbDataAdapter MyAd00 = new OleDbDataAdapter("select a0  from e61zrzy order by a0 desc ", MyConn);
        ...
        private void button8_Click(object sender, EventArgs e)
        {
            ...
            for (int v = 0; v < MyData.Tables[0].Rows.Count; v++)
            {
                ...
                if (this.a0.Text == MyData.Tables[0].Rows[v][0].ToString())
                {
                    ...
                    string SQL = "delete from e61zrzy where a0= '" + this.a0.Text + "'";
        ...
        {
            this.Enabled = true;
            Directory.CreateDirectory(".\\" + main.companyName + "\\wjgl\\561\\" + a0.Text + "");

            string temp = System.IO.Directory.GetCurrentDirectory();// d当前运行路径

            //    MessageBox.Show(temp);
            string OrignFile;//word模板路径
            OrignFile = "\\baseDB\\商标书式\\3 续展变更转让许可质押注销\\6质押\\1商标专用权质权登记申请书.dot";
        ...
        private void button10_Click(object sender, EventArgs e)
        {
            if (MyData.Tables[0].Rows.Count >= 1)
            {
                ...
                OleDbDataAdapter MyAd0 = new OleDbDataAdapter("select top 1 * from e61zrzy order by a0 desc", MyConn);
        ...
        {
            Process.Start(".\\" + main.companyName + "\\wjgl\\561");
        }
        ...
        {
            //内存数据删除
            ((DataSet)_561_f.MyData_delete).Tables[0].Rows.RemoveAt(_561_f.i);


            // int a =;// 数据库删除
            string SQL = "delete * from e61zrzy where a0='" + ((TextBox)_561_f.text_a0).Text + "'";
            OleDbCommand NewCom = new OleDbCommand(SQL, _561_f.MyConn);
            ...
            if (File.Exists(@".\\" + main.companyName + "\\wjgl\\561\\" + ((TextBox)_561_f.text_a0).Text + "\\" + ((TextBox)_561_f.text_a0).Text + ".docx"))
            {
                // MessageBox.Show("存在文件");
                File.Delete(@".\\" + main.companyName + "\\wjgl\\561\\" + ((TextBox)_561_f.text_a0).Text + "\\" + ((TextBox)_561_f.text_a0).Text + ".docx");

            }

            if (Directory.Exists(@".\\" + main.companyName + "\\wjgl\\561\\" + ((TextBox)_561_f.text_a0).Text + ""))
            {
                // MessageBox.Show("存在文件夹");
                Directory.Delete(@".\\" + main.companyName + "\\wjgl\\561\\" + ((TextBox)_561_f.text_a0).Text + "");
            }
